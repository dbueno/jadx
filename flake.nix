 {
  ...
  outputs = { self, nixpkgs, flake-utils, flake-compat, merjar }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        dex2jar-overlay = self: super: {
          dex2jar = super.dex2jar.overrideAttrs (old: {
            meta = old.meta or {} // {
              platforms = super.lib.platforms.all;
            };
          });
        };
        pkgs = import nixpkgs { overlays = [ dex2jar-overlay ]; inherit system; };
        apktool-lib = {
          lib
        }:
